...
23. **SNS wire format** — Query protocol (`POST /` with URL-encoded body and `Action=` parameter). Routed via SigV4 credential scope `service=sns`. Fully implemented. Subscriptions are auto-confirmed. SQS delivery is done via internal `sqs::enqueue_to_queue` (not HTTP). HTTP/HTTPS delivery is fire-and-forget (tokio::spawn + reqwest). Email delivery is log-only. Platform apps are stubbed as empty lists. Subscribe accepts `Attributes.entry.N.key/value` pairs (e.g. `RawMessageDelivery=true`).
...
26. **CloudWatch wire format** — Two separate service namespaces. **Metrics/alarms** (`service=monitoring`): Smithy RPCv2-CBOR protocol — `POST /service/GraniteServiceVersion20100801/operation/{OpName}` with CBOR bodies (via `ciborium` crate). Also has a `/cloudwatch/` direct route. Supported: PutMetricData, ListMetrics, GetMetricStatistics, PutMetricAlarm, DescribeAlarms, SetAlarmState, DeleteAlarms. Storage under `data/cloudwatch/metrics/{encoded_ns}/{metric}.json` and `data/cloudwatch/alarms/{name}.json`. `GetMetricStatistics` returns a single aggregated datapoint over the full time range. **Logs** (`service=logs`): JSON API with `X-Amz-Target: Logs_20140328.{Op}` header — routed via `top_level_dispatch` when target starts with `"Logs_"`. Supported: CreateLogGroup, DeleteLogGroup, DescribeLogGroups, CreateLogStream, DeleteLogStream, DescribeLogStreams, PutLogEvents (auto-creates group+stream), GetLogEvents, FilterLogEvents (substring match only). Storage under `data/cloudwatch_logs/groups/`, `streams/`, `events/`.

27. **IoT wire format** — REST/JSON. SigV4 credential scope `service=iot`. Routes are literal paths (`/things`, `/certificates`, `/policies`, etc.) — axum's literal-route priority ensures these beat S3's `/{bucket}` wildcard. Certificates are ECDSA P-256 self-signed X.509 generated via `rcgen`; certificate ID is the lowercase hex SHA-256 of the certificate PEM (64 chars). Private key is stored in the emulator for retrieval but is only returned on `CreateKeysAndCertificate`. MQTT broker is **not implemented** — `GetEndpoint` returns `localhost:8883` as a placeholder. Policy version IDs are numeric strings ("1", "2", …); version "1" is the default on creation. `AttachPolicy`/`DetachPolicy` use `PUT /target-policies/{policyName}` with body `{"target":"..."}`. `AttachThingPrincipal`/`DetachThingPrincipal` read the principal from the `x-amzn-principal` request header. Storage layout under `data/iot/`: `things/`, `thing_types/`, `certificates/`, `policies/`, `policy_versions/{name}/`, `policy_targets/{name}/`, `thing_principals/{name}/`.
